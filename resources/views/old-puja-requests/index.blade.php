        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="form-search-inline column-settings-inline pb-3">
                    <form class="form-inline form-search" method="GET" action="{{ route('old.puja.requests') }}" autocomplete="off">
                        <div class="row text-left" style="flex-wrap: nowrap;">
                            ...
                            <div class="col-4">
                                ...
                                <a href="{{ route('old.puja.requests') }}" class="btn btn-info mb-2">Reset</a>
                            </div>
                        </div>
                    </form>
                    <div class="clear"></div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ getClassNameFromStatus($order->request_status) }}">
                                <td><a href="{{ route('old.puja.requests.show', $order->order_id) }}">{{ $order->puja_request_id }}</a></td>
                                <td>{{ $order->payment_status }}</td>
                                <td>{{ formatDate($order->order_date) }}</td>
                                <td>
                                    {{ formatDate($order->date_of_puja) }}<br>
                                    {{ $order->from_time_of_puja }} - {{ $order->to_time_of_puja }}
                                </td>
                                <td>{{ $order->puja_names ?? 'N/A' }}</td>
                                ...
                                <td>
                                    <a href="{{ route('old.puja.requests.show', $order->order_id) }}" class="btn btn-default btn-xs" contenteditable="false" style="cursor: pointer;">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                </td>
                            </tr>
